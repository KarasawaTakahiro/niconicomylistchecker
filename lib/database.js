...
var database = {
    ...
    unwatch_movies: function (cb) {
        /*
         */
        this.each("SELECT * FROM items WHERE watched == 0 ORDER BY posted_at DESC", cb);
    },
    ...
    // マイリストに属する動画の未視聴数を取得
    unwatched_movie_num_at_feed: function(feed_id, cb){
        /*
         * cb(count)
         */
        this.get("SELECT count(*) FROM items WHERE watched = 0 AND feed_id = ?", feed_id, function(err, row){
            cb(row["count(*)"]);
        });
    },
    ...
    // -- UPDATE ----------------------------
    // 動画の視聴状況を変更
    change_watch_status: function (item_id, watch_status, cb) {
        this.run("UPDATE items SET watched = ?, updated_at = ? WHERE id == ?", watch_status, new Date(), item_id, cb);
    },
    ...
    // 指定した動画
    del_movie(movieid, feed_id, cb){
        this.run("DELETE FROM items where movieid = ? and feed_id = ?", movieid, feed_id, cb);
    },
