...
var database = {
    ...
    reg_feed: function(feed){
        ...
        this.reg_mylist(feed.mylistid, feed.title, feed.description, function(err){
            feed.items.forEach(function(item){
                reg_movie(item.movieid, item.title, item.posted_at, item.thumbnail, item.description, item.lastID);
            });
        });
    },
    ...
    change_feed_desc: function(feed_id, description, cb){
        db.run("UPDATE feeds SET description = ? WHERE id == ?", description, feed_id, cb);
    },

    // 動画の説明文を更新
    change_item_desc: function(item_id, description, db){
        db.run("UPDATE items SET description = ? WHERE id == ?", description, item_id, cb);
    },

    // フィードのタイトルを更新
    change_feed_title: function(feed_id, title, cb){
        db.run("UPDATE feeds SET title = ? WHERE id == ?", title, feed_id, cb);
    },
