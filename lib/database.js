...
var database = {
    ...
    del_feed_recursively(feed_id){
        ...
        this.movies_in_feed(feed_id, function(err, row){        // 紐付いた全動画取得
            if(!err){
                del_movie.call(parent_obj, row.id);
            }
        });
    ...
    // -- ラッパ -----------------------------
    // DBの初期化を待ってからrunを実行する
    run: function(sql){
        var timeout_cnt = 0;
        var TIMEOUT_MAX = 10;
        var parent_obj = this;          // 参照を保存しておく
        var args = arguments;           // このメソッドへの引数はDatabase.runに渡させるものなので退避しておく
        ...
        function waitForInitialize(){
            if(parent_obj.db == null){                  // 初期化前
                if(TIMEOUT_MAX < timeout_cnt){          // タイムアウト処理
                    console.log("abort: DB is closed.");
                    return;
                }
                setTimeout(waitForInitialize, 500);
                timeout_cnt += 1;
            }else{
                parent_obj.db.run.apply(parent_obj.db, args);
            }
        }
    },
